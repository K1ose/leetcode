...
public:
    ...
    long long maxTaxiEarnings_other(int n, vector<vector<int>> &rides)
    {
        vector<vector<pair<int, int>>> v(n + 1);
        for (auto &ride : rides)
        {
            v[ride[1]].push_back({ride[0], ride[2]});
        }
...
int main(int argc, char const *argv[])
{
    ...
    Solution slt;
    cout << slt.maxTaxiEarnings(n, rides);
    return 0;
}
