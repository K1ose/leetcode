...
public:
    ...
    {
        int n = energy.size(), res = 0, i = 0;
        while (i < n)
        {
            /* 如果精力和经验都超过对手，则击败对手，并扣除精力，获得经验 */
    ...
    /* 贪心+模拟 */
    int minNumberOfHours_greedy(int initialEnergy, int initialExperience, vector<int> &energy, vector<int> &experience)
    {
        int sum = accumulate(energy.begin(), energy.end(), 0);
        int res = max(0, sum - initialEnergy + 1);
...
int main(int argc, char const *argv[])
{
    ...
    Solution slt;
    cout << slt.minNumberOfHours(initialEnergy, initialExperience, energy, experience);
    return 0;
}
